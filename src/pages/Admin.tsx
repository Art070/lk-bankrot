...
function ClientWorkspace({ staffId }: { staffId: string }) {
  ...
  const loadClient = async (caseId: string) => {
    ...
    const [profile, details, payments, requests, notifications, messages, checkin] = await Promise.all([
      ...
      supabase.from('client_checkins').select('answers, updated_at').eq('case_id', caseId).maybeSingle(),
    ])
    const queryError = profile.error ?? details.error ?? payments.error ?? requests.error ?? notifications.error ?? messages.error ?? checkin.error
    if (queryError) setError(queryError.message)
    else setData({ caseRow, profile: profile.data ?? {}, details: details.data, payments: payments.data ?? [], requests: requests.data ?? [], notifications: notifications.data ?? [], messages: messages.data ?? [], checkin: checkin.data })
    setLoading(false)
  }
  ...
  const saveCaseAndProfile = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const casePatch = { case_number: value(form, 'caseNumber') || null, court: value(form, 'court') || null, case_status: value(form, 'caseStatus'), open_date: value(form, 'openDate'), next_hearing: value(form, 'nextHearing') || null, total_debt: Number(value(form, 'totalDebt') || 0), contract_number: value(form, 'contractNumber') || null, contract_date: value(form, 'contractDate') || null, payment_plan: value(form, 'paymentPlan'), contract_total: Number(value(form, 'contractTotal') || 0), remaining_payment: Number(value(form, 'remainingPayment') || 0), additional_expenses_note: value(form, 'additionalExpensesNote') || null }
